...
class cache:
	...
	#
	# Add address to cached addresses - 2.22
	#
	def cacheAddress (self, address):
		if address == "": return
		
		for cacheId in self.addressCache:
			if cacheId == address: return
	...
	#
	# Check if a device id is being monitored - 2.22
	#
	def addressInCache (self, address):
		for cacheId in self.addressCache:
			if cacheId == address: return True
	...
	def cacheDevices (self):
		...
		for dev in indigo.devices.iter("self"):
			...
			if deviceExists == False:
				...
				devProps["id"] = dev.id
				devProps["address"] = dev.address # 2.22
	...
	def addSubProps (self, sDev, varname):
		...
		self.cacheAddress (sDev.address)
		
		sProp = {}
		sProp["name"] = sDev.name
		sProp["id"] = sDev.id
		sProp["address"] = sDev.address # 2.22
